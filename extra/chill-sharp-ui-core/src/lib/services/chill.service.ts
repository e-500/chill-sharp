...
export class ChillService {
  ...
  private readonly canManagePermissionsState = signal(false);
  ...
  readonly canManagePermissions = this.canManagePermissionsState.asReadonly();
  ...
  async initialize(): Promise<void> {
    if (!this.isAuthenticated()) {
      this.canManagePermissionsState.set(false);
  ...
  getSchema(chillType: string, chillViewCode: string, cultureName?: string, update = false) {
    return this.chill.getSchema(chillType, chillViewCode, this.resolveCultureName(cultureName), update).pipe(
      map((response) => this.normalizeSchema(response)),
      catchError((error) => this.rethrowFriendlyError(error))
    );
  }

  setSchema(schema: ChillSchema) {
    const request: ChillDtoSchema = {
      ...
      properties: (schema.properties ?? []).map((property): ChillDtoPropertySchema => ({
        name: property.name,
  ...
  private async loadCurrentUserAccess(): Promise<void> {
    try {
      const response = await firstValueFrom(this.chill.getAuthPermissions());
      const isAuthenticated = this.isAuthenticated();
      this.canManagePermissionsState.set(isAuthenticated && response.user?.canManagePermissions === true);
      this.canManageSchemaState.set(isAuthenticated && response.user?.canManageSchema === true);
    } catch (error) {
      this.canManagePermissionsState.set(false);
      this.canManageSchemaState.set(false);
      console.warn('[ChillService] Unable to load current user access', error);
    }
  }
  ...
  private normalizeSchema(response: unknown): ChillSchema | null {
  ...
  private normalizeSchemaProperties(value: unknown): ChillPropertySchema[] {
    ...
    return value
      ...
      .map((property) => ({
        ...
        propertyType: this.readJsonNumber(property, 'PropertyType') ?? property.propertyType ?? CHILL_PROPERTY_TYPE.Unknown,
  ...
  private toSchemaRelationDtos(relations: ChillSchemaRelation[] | null | undefined): ChillDtoSchemaRelation[] {
    if (!Array.isArray(relations)) {
      return [];
    }

    return relations.map((relation): ChillDtoSchemaRelation => ({
  ...
  private serializeStringRecord(record: Record<string, JsonValue> | undefined): Record<string, string> {
    if (!record) {
      return {};
    }
