...
```csharp
var exampleExists = context.Entry(example)
    .Reference(x => x.Owner)
    .Exist();
```
...
Read the packaged `doc/ReferenceExistence.md` reference before using this for composite keys, shadow FKs, or legacy databases.
