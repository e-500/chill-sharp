...
- Server-side validation automatically executes authorization rules during CRUD actions on your context (`ChillSharpTemplateContext`).
- Clients can fetch user permissions via `/api/chill-auth/get-permissions` and evaluate access rules locally.

## 4. API Endpoints

Privileged users with the `CanManagePermissions` claim can manage ACL/auth settings via:
