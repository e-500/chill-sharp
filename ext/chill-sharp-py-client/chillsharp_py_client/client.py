...
class ChillSharpClient:
    ...
    def _apply_auth_token(self, payload: JsonDict, forget_password: bool) -> None:
        self._token_state.access_token = payload.get("AccessToken")
        self._token_state.access_token_issued_utc = self._parse_datetime(payload.get("AccessTokenIssuedUtc"))
        self._token_state.access_token_expires_utc = self._parse_datetime(payload.get("AccessTokenExpiresUtc"))
        self._token_state.refresh_token = payload.get("RefreshToken")
        self._token_state.refresh_token_expires_utc = self._parse_datetime(payload.get("RefreshTokenExpiresUtc"))

        if payload.get("UserName"):
            self._username = str(payload["UserName"]).strip()
